(function () {
  ...
  function buildAndAnimate(pFont, sFont) {
    ...
    var r1  = glyphPaths(pFont, "Brands we’ve", PAD, B1);
    var r2a = glyphPaths(sFont, 'grown',             PAD, B2);
    var r2b = glyphPaths(pFont, ' with.',            r2a.endX, B2);
    ...
    svg.setAttribute('class',       'work-svg-headline');
    ...
    /* Above the fold — play on load, not on scroll */
    if (document.readyState === 'complete' || document.readyState === 'interactive') {
      requestAnimationFrame(play);
    } else {
      document.addEventListener('DOMContentLoaded', function () { requestAnimationFrame(play); });
    }
  }
}());
